...
<div id="">
    ...
    <div class="tops1" style="padding-bottom:0">
        <section class="content contentseries" style="padding:16px 16px 0px;">
            <form action="<?php echo base_url();?>addepisode/<?php echo $this->uri->segment(3).'/'.$this->uri->segment(3); ?>" method="POST" enctype="multipart/form-data" id="display_result">
    		    <div style="display:flex; justify-content:center;">
    				<div class="" style="margin-bottom:20px; width:293px;">
    					<div class="row">
    						<label class="btn-default" style="background:none;width:293px;" for="upload-file-selector">
    							<div class="box box-widget widget-user boxshv" style="width:293px;">
    				...
    				<div class="" style="width:800px;">
						<div class="box box-widget widget-user boxshv seriesaddtodrafts" style="padding:0px 25px 25px; height:520px;">
    						...
    						<div class="row">
    							...
    							<div class="col-md-12">
        							<div class="box-body pad" style="margin-top:0px;padding-top: 0px;">
        								<textarea name="story" id="story" onclick="addseriesepisode(<?php echo $row->sid;?>)" class="draftsave form-control" placeholder="Enter Series Story Here...." required style="resize:none;height:360px;"><?php echo set_value('story');?></textarea>
...
var sidcount = 0;
    ...
    function previewContent(){
        var sid = $('#sid').val();
        if((sidcount == 0) && (sid == '' || sid == ' ' || sid == 'null')){
            var seriesid = $('input[name="series_id"]').val();
            addseriesepisode(seriesid);
            sidcount++;
        } if(sidcount > 0 && (sid != '' || sid!= 0 )) {
            var storytextarea = tinyMCE.activeEditor.getContent();
            var inputdata = $('.seriesaddtodrafts').find('input').serialize()+"&story="+storytextarea;
            $.ajax({
    			type: "POST",
    ...
    function submit(){
        ...
        if(title.length < 1){
        	...
        	setTimeout(function(){ $('#snackbar').removeClass('show'); }, 3000);
        }else if(storytext.length < 1){ //if(story.length < 200){
