<?php if(isset($loadcommustories) && ($loadcommustories->num_rows() > 0)){ foreach ($loadcommustories->result() as $key) { ?>
	<div class="box box-widget" style="background:#fff; border:1px solid #dddddd; margin-bottom:10px;" id="delcomm<?php echo $key->id;?>">        
        ...
        <span class="pull-right text-muted" onClick="comments(<?php echo $key->id;?>);">
        	...
        	<span id="old_cmt<?php echo $key->id;?>"><?php echo get_storycmtcount($key->id);?></span> Comments
        </span>
        </div>
        <?php } else { ?>
        <div class="box-header with-borders">
	...
	<div class="box box-widget" style="background:#fff; border:1px solid #dddddd; margin-bottom:10px;">
		<?php if(($topkey->type=='url')) { ?>
			<div class="box-header with-border">
				<div class="user-block">
					...
					<?php } ?>
					<span class="username">
					    <a href="<?php echo base_url().$this->uri->segment(1).'/'.$topkey->profile_name;?>"><?php echo $topkey->name;?></a>
	    ...
	    <?php } else { ?>
			...
			<div class="box-body">
				...
				<?php } ?>
				<span style="padding-left:5px;"><a data-toggle="modal" data-target="#soc" href="javascript:void(0);" style="color:#333;"><i class="fa fa-share"></i> Share</a></span>
				<span class="pull-right text-muted" onClick="comments(<?php echo $topkey->id;?>);">
					...
					} } ?><span id="old_cmt<?php echo $topkey->id;?>"><?php echo $commentcount;?></span> Comments 
				</span>
			</div>
		<?php } ?><!-- /.box-footer -->
		<div class="box-footer">
		...
		<!-- /.box-footer -->        					
        <div class="box-comments tajaxcomment<?php echo $topkey->id;?>" style="background: #fff;"> </div>
							...
							<?php $topsubcomments = get_subcomments($topkey->id, $comment->id); 
								...
								<div class="box-footer box-comments editdelete<?php echo $topsubcomment->id;?>" style="padding-bottom:0px;background:#ddd;">
								    <div class="box-comment">
								        ...
								        <div class="comment-text">
								            <span class="username">&nbsp;
								            <a href="<?php echo base_url().$this->uri->segment(1).'/'.$topsubcomment->profile_name;?>"><?php echo ucfirst($topsubcomment->name);?></a>
