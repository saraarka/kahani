...
class Malayalam extends CI_Controller {
    ...
    public function resetpassword(){
        $data1['languages'] = $this->User_model->language();
    	    $this->load->view('otherlangs/header', $data1);
	...
	public function series(){
		...
		$header['languages'] = $this->User_model->language();
		$this->load->view('otherlangs/header.php',$header);
		$this->load->view('otherlangs/series.php');
		$this->load->view('otherlangs/footer.php');
	}
    		...
    		if ($this->form_validation->run() == false) {
                ...
                if(isset($_FILES['cover_image']['name']) && !empty($_FILES['cover_image']['name'])){
                    ...
                    if(!$this->upload->do_upload('cover_image')){
                        ...
                        $this->load->view('otherlangs/header.php', $header);
                		$this->load->view('otherlangs/series.php', $error);
                        $this->load->view('otherlangs/footer.php');
	...
	public function series_priview($lid)
	{
		...
		$this->load->view('otherlangs/header.php', $header);
		$this->load->view('otherlangs/series_priview.php',$data);
        $this->load->view('otherlangs/footer.php');
  	}
	...
	public function addnewepisode($seriesnamesid, $seriesnamestoryid) {
		...
		$data['story_id'] = $s_storyid;
		$this->load->view('otherlangs/header.php', $header);
	    $this->load->view('otherlangs/newepisode.php',$data);
	    $this->load->view('otherlangs/footer.php');
	}
    ...
    public function story_info($sid){
		...
		$this->load->view('otherlangs/story_info',$data);
		//$this->load->view('otherlangs/footer');
	}
    ...
    public function series_edit($sid){
		...
		$this->load->view('otherlangs/header.php', $header);
        $this->load->view('otherlangs/series_edit.php',$data);
        $this->load->view('otherlangs/footer.php');
	}
	...
	public function updatestory12($id)
	{
		...
		$this->load->view('otherlangs/header.php', $data1);
    ...
    public function story(){
		...
		$header['languages'] = $this->User_model->language();
		$this->load->view('otherlangs/header.php', $header);
		$this->load->view('otherlangs/story.php');
        $this->load->view('otherlangs/footer.php');
	}
		...
		if ($this->form_validation->run() == false) {
		    ...
		    if(isset($_FILES['cover_image']['name']) && !empty($_FILES['cover_image']['name'])){
                ...
                if(!$this->upload->do_upload('cover_image')){
                    $error = array('error' => $this->upload->display_errors());
                    $this->load->view('otherlangs/header.php', $header);
            		$this->load->view('otherlangs/story.php', $error);
                    $this->load->view('otherlangs/footer.php');
                }else if($this->upload->do_upload('cover_image')){
                    $uploadData = $this->upload->data();
                    $config['image_library'] = 'gd2';
	...
	public function nano_story()
	{
		...
		$header['languages'] = $this->User_model->language();
	    $this->load->view('otherlangs/header.php', $header);
		$this->load->view('otherlangs/nano_story.php');
        $this->load->view('otherlangs/footer.php');
	}
	public function nano_insert(){
		...
		if ($this->form_validation->run() == false) {
		    $this->load->view('otherlangs/header.php', $header);
    		$this->load->view('otherlangs/nano_story.php',$data);
            $this->load->view('otherlangs/footer.php');
		}else{
    		$user_id = $this->session->userdata['logged_in']['user_id'];
    		$story = $this->input->post('story');
	...
	public function life() {
		...
		$data['tagslist'] = $this->User_model->lifetagslist();
		$this->load->view('otherlangs/header.php', $header);
		$this->load->view('otherlangs/life.php',$data);
		$this->load->view('otherlangs/footer.php');
	}
	...
	public function trans() {
	    if(isset($this->session->userdata['logged_in']['user_id']) && !empty($this->session->userdata['logged_in']['user_id'])){
    	    ...
    	    $this->load->view('otherlangs/header.php');
    ...
    public function commstoryview($commname, $comm_storyid){
        ...
        if(isset($commid) && !empty($commid)){
            ...
            $data['comm_comment_count'] = $this->User_model->comm_comment_count($commid);
            $data['commstory_likes'] = $this->User_model->usercommstory_likes();
	...
	//	$data['get_episode'] = $this->User_model->get_episode($sid);
	    $this->load->view('otherlangs/header.php', $header);
        $this->load->view('otherlangs/admin_story_view.php',$data);
        $this->load->view('otherlangs/footer.php');
    }
    ...
    public function viewallhome($type){
        if(($this->uri->segment(2) == "stories") && ($this->uri->segment(3) == 'latest')){
	        $type = 'story';
	    } else if(($this->uri->segment(2) == "series") && ($this->uri->segment(3) == 'latest')){
	        $type = 'series';
	...
	public function topviewallhome($type){
	    if(($this->uri->segment(2) == "stories") && ($this->uri->segment(3) == 'top')){
	        $type = 'story';
	    } else if(($this->uri->segment(2) == "series") && ($this->uri->segment(3) == 'top')){
	        $type = 'series';
    ...
    public function profileviewall() {
        ...
        if(isset($type, $pid) && !empty($pid) && in_array($type, array('series','story', 'life', 'nano'))){
            ...
            $data['followers_names'] = $this->User_model->followers_names($pid, 0,7);
            $data['following_names'] = $this->User_model->following_names($pid, 0,7);
    		$data['following'] = $this->User_model->following();
    		$data['type'] = $type;
    		$data['userid'] = $pid;
    		$this->load->view('otherlangs/header.php', $header);
