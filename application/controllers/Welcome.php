	...
	public function new_series_admin($seriesnamesid, $seriesnamestoryid) {
		...
		if(isset($_POST['seimage_sid']) && isset($sid) && !empty($sid)){
    		...
    		if(isset($data['new_series']) && ($data['new_series']->num_rows() > 0 )){
    		    ...
    		    }else if(($this->uri->segment(2) != '' && $this->uri->segment(3) != '') && ($this->uri->segment(2) != $urltitlecheck) || ($this->uri->segment(3) != $episodeurltitlecheck)){
                    $this->load->view('error');
    		    }
    		    /*if(($this->uri->segment(2) != '' && $this->uri->segment(3) != '') && ($this->uri->segment(2) != $urltitlecheck) || ($this->uri->segment(3) != $episodeurltitlecheck)){
                    $this->load->view('error');
    		    }*/
    		}
...
/*	public function updatestory12($id)
	...
	public function only_story_view($storynameid) {
		...
		if(isset($data['admin_story_view']) && ($data['admin_story_view']->num_rows() > 0 )){
		    ...
		    if($this->uri->segment(2) !='' && ($this->uri->segment(2) != $urltitlecheck)){
		        $this->load->view('error');
		    }
		    if(isset($metatags[0]->type) && ($metatags[0]->type == 'nano')){
		        $header['ogtitle'] = '[ Short Story ]';
            ...
            if(isset($_FILES['banner_image']['name']) && !empty($_FILES['banner_image']['name'])) {
                ...
                if(isset($picture1, $image) && !empty($picture1) && !empty($image)){
                    ...
                    if($res){
                        ...
                        redirect(base_url().'my_profile/'.$id);
                    }else{
                        redirect(base_url().$profilename);
                    }
                }else{
