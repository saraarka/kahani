			...
			<div class="book-type"><?php echo $vahseriesrow->gener;?></div>
				...
				<div>
					<font class="max-lines">
						<a href="<?php echo base_url('series/'.preg_replace("~[^\p{M}\w]+~u", '-', $vahseriesrow->title).'-'.$vahseriesrow->sid.'/'.preg_replace("~[^\p{M}\w]+~u", '-', $vahseriesrow->title).'-'.$vahseriesrow->story_id);?>" class="product-title">
							<?php echo $vahseriesrow->title;?>
						</a>
					</font> 
				</div>
			    <div class="flextest">
				    <font class="byname">By<font class="namehere"><a href="<?php echo base_url($vahseriesrow->profile_name);?>" style="color:#000">
				        <?php echo $vahseriesrow->name;?></a></font>
				    </font><br>
			    </div>
