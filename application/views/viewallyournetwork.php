    			...
    			<div>
    	            ...
    	            <?php foreach($viewallyournetwork->result() as $yournetworkrow) { ?>
            	        <div class="cardls">
    						...
    						<?php $yournwurl = base_url('story/'.preg_replace("~[^\p{M}\w]+~u",'-', $yournetworkrow->title).'-'.$yournetworkrow->sid);
        						$langfullname = get_langfullname($yournetworkrow->language); 
        						if(isset($langfullname) && !empty($langfullname)){
        						    $yournwurl = base_url($langfullname.'/story/'.preg_replace("~[^\p{M}\w]+~u",'-', $yournetworkrow->title).'-'.$yournetworkrow->sid);
        						} ?>
    						...
    						<div class="flextestls">
    							<font class="bynamels">By<font class="nameherels">
    							    <a href="<?php echo base_url().$yournetworkrow->profile_name; ?>"  style="color:#000"><?php echo $yournetworkrow->name;?></a></font></font><br>
    						</div>
...
<script>
    $(document).ready(function(){
        ...
        function load_country_data(limit, start) {
            $.ajax({
                url:'<?php echo base_url();?>welcome/viewallloadyournetwork',
                method:"POST",
