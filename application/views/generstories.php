    ...
    <?php foreach($loadgenerstories->result() as $loadgenerstory) { ?>
        <div class="cardls">
        	<div class="book-typels"><?php echo $loadgenerstory->gener;?></div>
                <a href="<?php echo base_url('story/'.preg_replace("~[^\p{M}\w]+~u", '-', $loadgenerstory->title).'-'.$loadgenerstory->sid);?>" class="imagesls-style">
            ...
            <div>
            	<font class="max-linesls">
            		<a href="<?php echo base_url('story/'.preg_replace("~[^\p{M}\w]+~u", '-', $loadgenerstory->title).'-'.$loadgenerstory->sid);?>" class="product-title"><?php echo $loadgenerstory->title;?></a>
            	</font> 
            </div>
        							
        	<div class="flextestls">
        		<font class="bynamels">By<font class="namehere"><a href="<?php echo base_url($loadgenerstory->profile_name);?>" style="color: #000;"> <?php echo $loadgenerstory->name;?></a></font></font><br>
        	</div>
