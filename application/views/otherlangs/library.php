                            ...
                            <div class="jc-m" style="display:flex; flex-wrap:wrap; margin-bottom:25px;">
                                ...
                                <div class="cardls" id="rllibdel<?php echo $rlseriesrow->sid;?>">
            						...
            						<div class="flextestls">
    					                <font class="bynamels">By<font class="nameherels"><a href="<?php echo base_url($this->uri->segment(1).'/'.$rlseriesrow->profile_name);?>" style="color:#000;"> <?php echo $rlseriesrow->name;?></a></font></font>
    					                <br>
            						</div>
            		    ...
            		    <div class="jc-m" style="display:flex; flex-wrap:wrap; margin-bottom:25px;">
                            <?php foreach($rlstories->result() as $rlstoryrow) { ?>
                    		    <div class="cardls" id="rllibdel<?php echo $rlstoryrow->sid;?>">
        							...
        							<div class="flextest">
        								<font class="byname">By<font class="namehere"><a href="<?php echo base_url($this->uri->segment(1).'/'.$rlstoryrow->profile_name);?>" style="color:#000;"> <?php echo $rlstoryrow->name;?></a></font></font>
        								<br>
        							</div>
...
<script>
    function deleterllibrary(story_id){
        ...
        $('#confirmdelpopup').modal().one('click', '#delconfirmed', function (e) {
            $.ajax({
                type:"post",
                url:"<?php echo base_url($this->uri->segment(1).'/deletelibrary');?>",
	...
	function subscribedstories(){
	    $.ajax({
			type:"post",
			url:"<?php echo base_url($this->uri->segment(1).'/seriessubscribe');?>",
			success:function(data){
			    $('#tab_2').html(data);
			}
		});
	...
	function favoritestories(){
	    $.ajax({
			type:"post",
			url:"<?php echo base_url($this->uri->segment(1).'/storiesfavorite');?>",
			success:function(data){
			    $('#tab_3').html(data);
			}
		});
