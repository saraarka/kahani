<?php if(isset($viewallyournetwork) && ($viewallyournetwork->num_rows() >0)){ foreach($viewallyournetwork->result() as $loadyournetwork) { ?>
    <div class="cardls">
		...
		<?php $yournwurl = base_url('story/'.preg_replace('/\s+/', '-', $loadyournetwork->title).'-'.$loadyournetwork->sid);
			$langfullname = get_langfullname($loadyournetwork->language); 
			if(isset($langfullname) && !empty($langfullname)){
			    $yournwurl = base_url($langfullname.'/story/'.preg_replace('/\s+/', '-', $loadyournetwork->title).'-'.$loadyournetwork->sid);
			} ?>
		...
		<div class="flextestls">
			<font class="bynamels">By<font class="nameherels">
			    <a href="<?php echo base_url().$this->uri->segment(1).'/'.$loadyournetwork->profile_name; ?>" style="color:#000"> <?php echo $loadyournetwork->name;?></a></font></font><br>
		</div>
