        ...
        <div class="write-nav">
            ...
            <div class="dropdown-content1" id="dropdown">
                ...
                if(isset($row->sid) && ($row->sid != $row->story_id) && ($row->type == "series")){
                    ...
                    <?php }else{ ?>
                        <a href="<?php echo base_url($this->uri->segment(1).'/admin-series/'.preg_replace("~[^\p{M}\w]+~u", '-', $row->title).'-'.$row->sid.'/'.preg_replace("~[^\p{M}\w]+~u", '-', $seriesftitles).'-'.$prefaceid);?>" style="display:flex;font-family: Arial, sans-serif;">
                            <div class="episode-div"><?php echo $i; ?></div>     
                            <div style="padding: 0px 8px;">
                                <div class="episode-name"><?php echo ucfirst($row->title); ?></div> 
                                <div class="episode-date"><?php echo date("M j, Y", strtotime($row->date));?>
...
<script>
    ...
    function serverurldraftimage(picture1, story_id, type){
        $.ajax({
            url: "<?php echo base_url().$this->uri->segment(1);?>/serverurldraftimage",
            type: 'POST',
    ...
    function urldraftimage(picture1, image, story_id){
        $.ajax({
            url: "<?php echo base_url().$this->uri->segment(1);?>/urldraftimage",
            type: 'POST',
